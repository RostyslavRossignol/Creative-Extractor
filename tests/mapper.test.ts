...
test("extracts only the act parameter from an Ads Manager URL", () => {
  const url = "https://adsmanager.facebook.com/adsmanager/manage/campaigns?global_scope_id=415037337992011&business_id=415037337992011&act=1763452808338012&nav_entry_point=am_global_scope_selector";
  assert.equal(extractAdAccountId(url), "1763452808338012");
  assert.equal(extractAdAccountId("act_1763452808338012"), "1763452808338012");
  assert.equal(extractAdAccountId("1763452808338012"), "1763452808338012");
  assert.equal(extractAdAccountId("business_id=415037337992011"), "");
});

test("checks ad account access before upload and explains an inaccessible account", async () => {
  const originalFetch = globalThis.fetch;
  const logs: unknown[] = [];
  let calls = 0;
  globalThis.fetch = (async () => {
    ...
    return new Response(JSON.stringify({
      error: {
        message: "Unsupported post request. Object with ID does not exist or is missing permissions.",
  ...
  try {
    await assert.rejects(
      verifyAdAccountAccess({ accountId: "415037337992011", token: "secret-token", onLog: (entry) => logs.push(entry) }),
      /Токен не имеет доступа.*act_415037337992011.*неверный ID/,
    );
...
test("limits parallel Meta uploads to the configured concurrency", async () => {
  let active = 0;
  let maximum = 0;
